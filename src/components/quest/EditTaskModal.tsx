...
interface EditTaskModalProps {
    quest: Quest;
    onSubmit: (questId: string, title: string, description: string) => Promise<void>;
    onClose: () => void;
}

export function EditTaskModal({ quest, onSubmit, onClose }: EditTaskModalProps) {
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full max-w-lg rounded-2xl border border-surface-200 bg-white p-6 shadow-elevated animate-fade-slide-up">
                ...
                {/* Header */}
                <div className="mb-5">
                    <h3 id="edit-task-modal-title" className="text-lg font-semibold text-surface-900">
                        Edit Task Proposal
                    </h3>
                    <p className="mt-1 text-sm text-surface-500">
                        Update the title or description of your pending task.
                    </p>
                </div>

                {/* Edit History */}
                {quest.editHistory && quest.editHistory.length > 0 && (
                    <div className="mb-4 rounded-lg border border-surface-100 bg-surface-50 p-3">
                        <p className="text-xs font-medium text-surface-600 mb-2">Edit History</p>
                        <div className="space-y-2 max-h-32 overflow-y-auto">
                            {quest.editHistory.map((entry, idx) => (
                                <div key={idx} className="text-xs text-surface-500 border-l-2 border-surface-200 pl-2">
                                    <span className="font-medium">{entry.title}</span>
                                    <span className="text-surface-400 ml-2">
                                        {entry.editedAt.toLocaleDateString()}
                                    </span>
                                </div>
                            ))}
                        </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="edit-task-title" className="block text-sm font-medium text-surface-700">
                        ...
                        <input
                            ...
                            aria-describedby="edit-title-error edit-title-count"
                            aria-invalid={titleTouched && !titleValidation.valid}
                            autoFocus
                        />
                        <div className="mt-1.5 flex items-center justify-between">
                            <div id="edit-title-error">
                                {titleTouched && <ValidationError message={titleValidation.error} />}
                            </div>
                            <span id="edit-title-count" className="text-xs text-surface-400">
                                {title.length}/100
                            </span>
                        </div>
                    </div>
                    ...
                    <div>
                        <label htmlFor="edit-task-description" className="block text-sm font-medium text-surface-700">
                        ...
                        <textarea
                            ...
                            className="mt-1.5 block w-full rounded-lg border border-surface-200 bg-surface-50 px-3 py-2.5 text-sm text-surface-900 placeholder-surface-400 focus:border-madrid-500 focus:outline-none focus:ring-1 focus:ring-madrid-500"
                            aria-describedby="edit-desc-error edit-desc-count"
                            aria-invalid={descTouched && !descValidation.valid}
                        />
                        <div className="mt-1.5 flex items-center justify-between">
                            <div id="edit-desc-error">
                                {descTouched && <ValidationError message={descValidation.error} />}
                            </div>
                            <span id="edit-desc-count" className="text-xs text-surface-400">
